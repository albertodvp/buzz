...
Fresh-start discovery is deliberately bounded. Desktop queries the selected
channel plus at most eight channels selected from its existing activity/read
models; selecting any other channel discovers it on demand. The concurrency
scheduler remains a secondary guard, not the bound itself. Mobile batches the
first page for known channels, exposes per-channel cursor pagination, and uses a
single multi-channel live subscription to refresh reply and deletion changes.
...
- a separate cross-channel thread inbox.
