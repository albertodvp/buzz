  ...
  Widget build(BuildContext context, WidgetRef ref) {
    ...
    return Column(
      ...
      children: [
        ...
        if (threadSidebar.hasMore(channel.id))
          SizedBox(
            height: 44,
            child: TextButton(
              onPressed: threadSidebar.isLoadingMore(channel.id)
                  ? null
                  : () => unawaited(
                      ref
              ...
              child: threadSidebar.isLoadingMore(channel.id)
                  ? const SizedBox.square(
                      dimension: 16,
                      child: CircularProgressIndicator(strokeWidth: 2),
                    )
                  : const Text('Load more threads'),
            ),
          ),
      ],
    );
